...
<h1>Введите в поле адресс сайта, который нужно проверить</h1>
<form method = "post" >
...
echo 'Состояние: ' . 'В файле прописано ' . $this->termsCount . ' директив Host' . "<br/>";
echo '<hr>';
echo 'Рекомендации: ' . 'Программист: Необходимо добавить в файл robots.txt директиву Host. Директива Host задётся в файле 1 раз, после всех правил.' . "<br/>";
}
...
echo 'Состояние: ' . 'Размера файла robots.txt составляет ' . $this->robotFSize . ' байт, что находится в пределах допустимой нормы' . "<br/>";
echo '<hr>';
echo 'Рекомендации: ' . 'Доработки не требуются' . "<br/>";
}

if($this->testresults[$i] == "Проверка размера файла robots.txt" && $this->status[$i] == 0){
echo 'Состояние: ' . 'Размера файла robots.txt составляет ' . $this->throbotFSize . ' байт, что превышает допустимую норму' . "<br/>";
echo '<hr>';
echo 'Рекомендации: ' . 'Программист: Максимально допустимый размер файла robots.txt составляет 32 кб. Необходимо отредактировть файл robots.txt таким образом, чтобы его размер не превышал 32 Кб' . "<br/>";
}



if($this->testresults[$i] == "Проверка кода ответа сервера для файла robots.txt" && $this->status[$i] == 1){
echo 'Состояние: ' . 'Файл robots.txt отдаёт код ответа сервера: ' . $this->serverAnswer . "<br/>";
echo '<hr>';
echo 'Рекомендации: ' . 'Доработки не требуются' . "<br/>";
}

if($this->testresults[$i] == "Проверка кода ответа сервера для файла robots.txt" && $this->status[$i] == 0){
echo 'Состояние: ' . 'При обращении к файлу robots.txt сервер возвращает код ответа: ' . $this->serverAnswer . "<br/>";
echo '<hr>';
echo 'Рекомендации: ' . 'Программист: Файл robots.txt должны отдавать код ответа 200, иначе файл не будет обрабатываться. Необходимо настроить сайт таким образом, чтобы при обращении к файлу robots.txt сервер возвращает код ответа 200' . "<br/>";
}
